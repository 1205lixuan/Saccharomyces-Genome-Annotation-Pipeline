...
my $infile  = shift;#fna_list file in the working directory. all fna files should be placed in the genome_dir
if (not $infile){
	die "usage: perl annt_cluster_pipeline.plx <query_fa_list>\n";
}#if
...
my $cov = 50;
my $iden = 50;
my $runpref = "rundiamondc50i50test3";
my $fampref = "DSCF50TEST3";
...
system ("perl $script_dir/maker_clust2_diamond.plx $cpu_num $cov $iden $ref_dir $ref_prot_faa $genome_dir $infile $runpref");
# 2.5. create families based on the information obtained above
